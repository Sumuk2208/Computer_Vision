...
def apply_homography(h: np.ndarray, points: np.ndarray) -> np.ndarray:
    ...
    """
    # Hint: augment the points, and use the '@' numpy operator for matrix multiplication
    ones = np.ones((1, points.shape[1]))
    augmented_points = np.vstack((points, ones))
    transformed_points = h @ augmented_points
    new_points = transformed_points[:2] / transformed_points[2]
...
def calculate_bounding_box_of_warps(homographies, images):
    """Compute bounding box ensuring correct width and height order."""
    new_corners = []
    for h, im in zip(homographies, images):
        ...
        new_corners.append(apply_homography(h, corners_in_image_coordinates))

    warped_corners = np.hstack(new_corners)
    all_x_values, all_y_values = warped_corners[0], warped_corners[1]
...
def weighted_blend(images: list[np.ndarray]) -> np.ndarray:
    ...
    ans = np.sum([w * im for w, im in zip(weights, images)], axis=0)
    return ans / sum_of_weights
...
def stitch(
    ...
    x_shift, y_shift = -min(0, left), -min(0, top)
    ...
    # Warp images into (width, height) instead of (height, width)
